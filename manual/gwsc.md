...
interaction `W`. It is now specified in **two TOML files**:

| where | what |
|---|---|
| `[product_basis]` in `ctrlg.<sname>.toml` | global cut-offs: `pb_tolerance`, `pb_lcutmx` |
| `PB.<sname>.toml`              | per-atom radial-function tables (`nlx`, `valence`, `core`) |

`PB.<sname>.toml` is auto-emitted by `ctrlgenToml.py` / `Legacy2toml.py`
and almost never needs hand editing. When you tune a GW calculation,
...
## `PB.<sname>.toml` (per-atom tables)

The per-spec radial-function tables (`nlx`, `valence`, `core`) live in
the sibling `PB.<sname>.toml`. One block per species, e.g.

```toml
[[spec]]
atom = "Cu"

# Number of radial functions per (l, type). Do not edit by hand --
# auto-derived from rsmh / eh / pz / lo of [[spec]] in ctrlg.<sname>.toml.
nlx = [
  # l   nnvv  nnc
  [ 0,    2,    3 ],
  [ 1,    2,    2 ],
...
# Valence radial functions selected for the product basis.
# columns: l, n, occ, unocc, label
# n=1 -> phi, n=2 -> phidot, n=3 -> phiz (local orbital)
valence = [
  [ 0, 1, 1, 1, "4s_phi"  ],
  [ 0, 2, 0, 0, "4s_phidot" ],
  [ 1, 1, 1, 1, "4p_phi"  ],
...
# Core radial functions (kept for compatibility; usually all-zero in modern runs).
# columns: l, n, occ, unocc, ForX0, ForSxc, label
core = [
  [ 0, 1, 0, 0, 0, 0, "1S" ],
  [ 0, 2, 0, 0, 0, 0, "2S" ],
  [ 0, 3, 0, 0, 0, 0, "3S" ],
...
- `nlx[l] = [l, nnvv, nnc]` — how many radial functions ecalj used for
  valence (`nnvv`) and core (`nnc`) on each `l` channel. `nnvv = 2` is
  `{phi, phidot}`; `nnvv = 3` adds the local orbital `phiz`.
- `valence` chooses **which** radial functions feed the product basis.
  `occ` and `unocc` are 0/1 switches; ecalj forms products
  `occ × unocc` and drops near-linearly-dependent ones (controlled by
  `pb_tolerance`). The two groups correspond loosely to occupied vs
  unoccupied subspaces. `phidot` is usually excluded for performance
  (set both columns to 0) but a more accurate basis includes it.
- `core` — historical, obsolete. Modern runs leave it all-zero. The
  `ForX0` / `ForSxc` columns came from the CORE1/CORE2 distinction in
  the 2007 paper; CORE2 is no longer used.
