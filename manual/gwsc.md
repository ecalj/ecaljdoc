...
The per-atom radial-function tables (`nlx`, `valence`, `core`) live in
the sibling `PB.<sname>.toml`. It is one flat `[product_basis]` table whose
rows all start with the site index `iatom` (in `[[site]]` order). `gwinit`
writes it from `rsmh`/`eh`/`pz` of `[[spec]]` and adds a label per row;
the explanation below is written into the file itself as comments.

```toml
# PB.<sname>.toml -- auto-generated by gwinit
[product_basis]
