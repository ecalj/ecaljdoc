...
> **`PB.toml` is for the GW path** (it feeds the product-basis
> generation in `hbasfp0` / `hvccfp0`). It is auto-generated by
> `ctrlgenToml.py` (or `Legacy2toml.py`) and **does not normally need
> hand editing**. Edits live in `ctrlG.<sname>.toml` for almost all
> use cases.
