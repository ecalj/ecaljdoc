...
> ⚠️ **TOML migration (2026-05)** — GPU 版 `gwsc` も `ctrlg.<sname>.toml` を読みます。Legacy `ctrl.<sname>` / `GWinput` directory は `Legacy2toml.py <sname>` で変換してから実行してください。See [TOML migration](./toml_migration).
