...
ctrl contains basis inputs to invoke lmf,lmfa, and lmchk.
ctrl has `Category_Token` structures. It is easier to understand it by examining the ctrl files generated by ctrlgenM1.py.

# `ctrl`
ctrl is the basic input file.  It is better to look into template of ctrl file generated by ctrlgenM1.py. See [Tutorial](../manual/README_tutorial.md).

> <small>When we run fortran programs (lmf, lmfa, lmchk),
we run ecalj/SRC/exec/ctrl2ctrlp.py (after copied to your BINDIR)
to convert ctrl.foobar to ctrlp.foobar. See ~/ecalj/SRC/main/laf.f90 for example.
Then we read ctrlp by SRC/subroutines/m_lmfinit.f90.</small>
...
Here is the list of Categoy_Token. Note that those with `.` at top is only for developers.
To see full list, look into `m_lmfinit.f90` which reads in ctrlp file.
...
```
 ...
 STRUC_ALAT        reqd   r8       1,  1      Units of length (atomic unit, a.u.)
