...
You can see
 ...
 2. A line beginning with a `%const' is to define variables used in ctrl file.


ctrl is the basic input file.  It is better to look into template of ctrl file generated by ctrlgenM1.py. Generated ctrl file is self-documented. See [Tutorial](../manual/README_tutorial.md).

> <small>When we run fortran programs (lmf, lmfa, and lmchk),
we run ecalj/SRC/exec/ctrl2ctrlp.py (at your BINDIR) from the fortran program in order
to convert ctrl.foobar to ctrlp.foobar. See ~/ecalj/SRC/main/lmf.f90 for example.
...
```
  ...
  SITE_ATOM_RELAX   opt    i4v      3,  1          default= 1 1 1
    ...
    This is only for DYN mode (relax). If RELAX=0 0 1, only z direction relaxiation.
```
