...
その `Worb` も**実質自動化できる**。ミニマム模型で取るのは「各原子が基底に
持っている価電子チャネル」であって、`gwinit` は既にその雛形を書き出している
(§1)。手で書くのは、d だけ・4f だけと**狙って絞るとき**(§3)か、
真空層に空格子球を足すとき(§4)に限られる。
...
書くが、**調整ノブではなく模型の定義そのもの**であり、method に関係なく
MLO には常に必要なものである。

そして**これ自体も実質自動化できる。** ミニマム模型で取りたいのは
「各原子が基底に持っている価電子チャネル」であって、それは基底の組み方から
既に決まっているからである。実際 `gwinit` は全原子 × s,p,d の行を
そのまま雛形として書き出している:

```toml
Worb = """
! 1 Fe   1 2 3 4 5 6 7 8 9
! 2 O    1 2 3 4 5 6 7 8 9
"""
```

`!` を外せばそれがミニマム模型になる。手が要るのは次の 2 つだけで、
どちらも**自動化できない種類の判断ではなく、そもそも別の模型を作りたい**場合である:

- **狙って絞るとき** — d だけ、4f だけを取り出す(§3)。何を取るかは物理の選択
- **空格子球を足すとき** — 真空層を持つスラブ(§4)。原子の無い場所に
  基底を置くのだから、構造から自動では出てこない
...
両者がどれだけ重なるかを測る。**物質ごとの調整は無い** — `Worb` も
価電子チャネルをそのまま取ればよい(§1)。
...
物質ごとの入力は `Worb` だけで(それも雛形どおり、§1)、`mlo_emax` は
どのサンプルからも消えた。
