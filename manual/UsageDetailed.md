...
With the GW driver settings (now in the `[gw]` / `[mlo]` / `[blocks]` / `[product_basis]` sections of `ctrlg.<sname>.toml`) generated by `job_materials.py NiO`, we can run gwsc as
```
gwsc -np 32 5 nio 
```
...
## LDA+U
We have samples 
* https://github.com/tkotani/ecalj/tree/main/Samples/Legacy/GdNldau
